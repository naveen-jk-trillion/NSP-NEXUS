...
export const CHANNEL = "NSP NEXUS_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "NSP NEXUS_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "NSP NEXUS_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _NSP_NEXUSLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _NSP_NEXUSLastMsg) return; // identical – drop
    _NSP_NEXUSLastMsg = key;
  } catch {
...
// Helper to parse data-NSP NEXUS-id to extract file path, line, and column
const parseNspNexusId = (
  nspNexusId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = nspNexusId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-NSP NEXUS-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-NSP NEXUS-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the NSP NEXUS ID from the element to ensure we're working with the right one
    const nspNexusId = element.getAttribute("data-NSP NEXUS-id");
    if (!nspNexusId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseNspNexusId(nspNexusId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same NSP NEXUS ID
    const allMatchingElements = document.querySelectorAll(
      `[data-NSP NEXUS-id="${nspNexusId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(nspNexusId) || {};
    appliedStylesRef.current.set(nspNexusId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(nspNexusId);
  ...
  const flushImageSrcChange = () => {
    ...
    const nspNexusId = imgElement.getAttribute("data-NSP NEXUS-id");
    if (!nspNexusId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "NSP NEXUS_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same NSP NEXUS ID
        const allMatchingElements = document.querySelectorAll(
          `[data-NSP NEXUS-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "NSP NEXUS_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-NSP NEXUS-id
        const nspNexusId = element.getAttribute("data-NSP NEXUS-id");
        if (nspNexusId) {
          const parsed = parseNspNexusId(nspNexusId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-NSP NEXUS-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-NSP NEXUS-id");
        ...
        const tagName =
          hit.getAttribute("data-NSP NEXUS-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same NSP NEXUS ID
        const allMatchingElements = document.querySelectorAll(
          `[data-NSP NEXUS-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-NSP NEXUS-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-NSP NEXUS-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same NSP NEXUS ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-NSP NEXUS-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-NSP NEXUS-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same NSP NEXUS ID
        const allMatchingElements = document.querySelectorAll(
          `[data-NSP NEXUS-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-NSP NEXUS-name") ||
